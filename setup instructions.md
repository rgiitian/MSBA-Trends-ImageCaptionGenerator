...
Original author: https://github.com/dabasajay/Image-Caption-Generator

![amazson_S3_logo_jpg.jpg](attachment:amazson_S3_logo_jpg.jpg)

## Implementing Amazon Webservices
...
### Packages used for implementing the code
...
from utils1.model import CNNModel, generate_caption_beam_search
import os
import pandas as pd
...
## 2. Code to connect Amazon S3 bucket to Local system


```python
###Loading the files from S3 bucket

bucket_name = 'msba-trends-bigdata-2022'
folder_key = 'Caption-Generator-Test/'

def s3_bulk_download(bucket_name, folder):

    s3 = boto3.resource(
        service_name='s3',
        region_name='us-east-1',
        aws_access_key_id='insert key_id',
        aws_secret_access_key='insert password key'
    )
    ...
    for file in my_bucket.objects.filter(Prefix = folder):
        if '.jpg' in file.key:
            ...
            my_bucket.download_file(file.key, config['test_data_path'] + filename)
            

# Download files from S3 bucket to test folder
s3_bulk_download(bucket_name, folder_key)
```

## To perform Webcam Live demo


```python
##To perform live demo using webcam using OpenCV module

def capture_image_cam(foldpath):
    ...
    while True:
        ...
        if k%256 == 27:
            ...
            # SPACE pressed
            img_name = foldpath + "opencv_frame_{}.jpg".format(img_counter)
#             img_name = "/Users/rithwiksivadasan/Desktop/test_loc/opencv_frame_{}.png".format(img_counter)
...
# capture_image_cam(config['test_data_path'])
```
